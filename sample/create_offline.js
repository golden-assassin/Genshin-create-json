...
async function dataSetup(uid,select=0,reference) {
  const response = await request(uid);
  if (!response) return null;
  const [user, character, loc] = response;
  const result = artifact_Calculation(select, reference, user, loc);
  const user_character = user.avatarInfoList[select];
  const constellation = user_character.talentIdList || [];
  const avatarId = user.playerInfo.showAvatarInfoList[select].avatarId;
  const elements = {"Fire": "炎","Water": "水","Wind": "風","Electric": "雷","Rock": "岩","Ice": "氷","Grass": "草"};
  const element_name = elements[character[avatarId.toString()].Element];
  ...
  const output_json = {
    ...
    Character: {
      ...
      Level: user.playerInfo.showAvatarInfoList[select].level,
...
function artifact_Calculation(select,reference,user=null,loc=null) {
  if (user && user.avatarInfoList && Object.values(user.avatarInfoList).length > 0) {
    const first_avatar_info = user.avatarInfoList[select];
...
async function request() {
  const user = load('user.json');
  const character = load('characters.json');
  const loc = load('loc.json');
  return [user, character, loc];
}

const reference = ["攻撃力","防御力","HP","元素熟知","元素チャージ効率"];
dataSetup(uid=114514810, select=0, reference[0]).then(data => {
